CI/CD process for deploying and managing a data platform is visible in multiple places. Starting from infrastructure, there should be no place for manually created server, network or any other resources, with tools like Terraform or Ansible managing all of it. Second CI/CD layer relates to application part, where all changes within containers running on a cluster should be tested and prepared in an automated way. Final place for CI/CD is within data flows that are going to be executed on orchestrator like Prefect, Apache Airflow or Dagster. This part shouldn't allow manual changes with everything being hosted on repository like GitHub or Gitlab.

# Overview
1. [Infrastructure as Code (IaC)](#iac)
    1. [Terraform for Cloud Environments](#iac_terraform)
        1. [Benefits of Using Terraform](#iac_terraform_benefits)
        2. [Setting Up a CI/CD Pipeline with Terraform](#iac_cicd_setup)
        3. [Example CI/CD Workflow with Terraform and GitHub Actions](#iac_cicd_example)
        4. [Terraform best practices](#iac_best_practices)
    2. [Ansible for On-Premises Environments](#ansible)
        1. [What is Ansible?](#ansible_overview)
        2. [Benefits of Using Ansible](#ansible_benefits)
        3. [Setting Up a CI/CD Pipeline with Ansible](#ansible_cicd_setup)
        4. [Example CI/CD Workflow with Ansible and Gitlab CI/CD](#ansible_cicd_example)
2. [Application Deployment Automation](#cicd)
3. [Data Flow Automation](#flow)
